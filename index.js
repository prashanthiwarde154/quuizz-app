...
const roomUsers = {};
const userAnswers = {};
let currentQuestionIndexMap = {};

const questionsByCategory = {
  "Computer Science": [
    { question: "What does CPU stand for?", options: ["Central Processing Unit", "Computer Processing Unit", "Central Performance Unit", "Control Program Unit"], answer: "Central Processing Unit" },
    { question: "What is RAM used for?", options: ["Long-term storage", "Display", "Short-term memory", "Power supply"], answer: "Short-term memory" }
  ],
  "General Knowledge": [
    { question: "What is the capital of India?", options: ["Mumbai", "Delhi", "Chennai", "Kolkata"], answer: "Delhi" },
    { question: "Who is the current Prime Minister of India?", options: ["Rahul Gandhi", "Narendra Modi", "Amit Shah", "Manmohan Singh"], answer: "Narendra Modi" }
  ],
  "DSA": [
    { question: "Which data structure works on the principle of FIFO?", options: ["Stack", "Queue", "Tree", "Graph"], answer: "Queue" },
    { question: "Which data structure works on the principle of LIFO?", options: ["Stack", "Queue", "Heap", "Graph"], answer: "Stack" }
  ],
  "Output Prediction": [
    { question: "What will be the output of: console.log(2 + '2');", options: ["22", "4", "Error", "undefined"], answer: "22" },
    { question: "What will be the output of: console.log(typeof null);", options: ["null", "object", "undefined", "string"], answer: "object" }
  ],
  "Syntactical Error": [
    { question: "Find the syntax error: console.log('Hello World)", options: ["Missing semicolon", "Missing closing quote", "Incorrect function name", "Extra bracket"], answer: "Missing closing quote" },
    { question: "Find the syntax error: if(x = 5) { console.log(x); }", options: ["Use of '=' instead of '==' or '==='", "Missing parenthesis", "Extra semicolon", "Invalid variable name"], answer: "Use of '=' instead of '==' or '==='" }
  ],
  "Basic Programming": [
    { question: "Which symbol is used to end a statement in C, C++, and Java?", options: [";", ":", ".", ","], answer: ";" },
...
app.post('/api/create-room', (req, res) => {
  ...
  if (rooms.has(roomCode)) return res.status(500).json({ error: 'Room code collision. Try again.' });
...
io.on('connection', (socket) => {
  ...
  socket.on('join-room', (roomCode) => {
    ...
    if (!roomUsers[roomCode].includes(socket.id)) roomUsers[roomCode].push(socket.id);
    socket.to(roomCode).emit('user-joined', socket.id);

    if (roomUsers[roomCode].length === 2) io.to(roomCode).emit('room-ready');
  });
  ...
  socket.on('start-game', ({ roomCode, category }) => {
    ...
    roomDataMap[roomCode] = { allAnswers: [], questions, scores: {} };
    const totalQuestions = questions.length;

    const sendNextQuestion = () => {
      const currentIndex = currentQuestionIndexMap[roomCode];
      const question = questions[currentIndex];
      if (question) {
        userAnswers[roomCode][currentIndex] = [];
        ...
        if (roomUsers[roomCode]?.length > 0) {
          setTimeout(() => {
            ...
            if (currentQuestionIndexMap[roomCode] < questions.length) sendNextQuestion();
            else sendFinalResults(roomCode);
          }, 30000);
        }
      } else {
  ...
  socket.on('submit-answer', ({ roomCode, questionIndex, username, selectedOption }) => {
    ...
    if (answersArray.some(ans => ans.username === username)) return;
    ...
    if (selectedOption === correctAnswer) userScores[roomCode][username] += 1;

    if (roomDataMap[roomCode]) {
      const players = (roomUsers[roomCode] || []).map(id => io.sockets.sockets.get(id)?.username || 'User');
...
function sendFinalResults(roomCode) {
  io.to(roomCode).emit('quiz-ended', {
    ...
    players: (roomUsers[roomCode] || []).map(id => io.sockets.sockets.get(id)?.username || 'User'),
